...
| **Face ID** | Register a lifter in about six seconds from a browser on the LiftGuard computer. Sessions on that computer's camera then start under their name, and anyone else trains as Guest. Only a face signature (128 numbers, not a photo) is stored, on that computer; it never leaves it. See [docs/FACE_ID.md](docs/FACE_ID.md). |
...
- **Face recognition** uses OpenCV Zoo's YuNet + SFace models (MIT / Apache-2.0; downloaded with `backend/fetch_face_models.py`, not in the repo). SFace's training data is not stated by OpenCV Zoo; the paper used research celebrity-photo datasets, so it needs review before any paid release. Fine for telling apart a few registered lifters on one machine. Not an identity or security check.
...
Push-up is next. Its engine is in place: elbow-angle rep counting, gates that reject standing arm bends and sliding hands, and flags for shallow reps, sagging or piking hips, depth drift and fast descent. So far it is tested only on synthetic skeletons, so the app lists it as "Coming" and it can't be picked until it has been checked on a recorded clip. The mode picker is already in Settings and on the Live screen. Lunge is built the same way (front-knee counting for stationary lunges, with flags for shallow depth, forward lean, knee past toes as a measured distance, depth drift and fast descent), also tested only on synthetic skeletons and not selectable yet. Jumping jacks is built too (front view: arm-angle counting, gates for both arms together and feet jumping apart, flags for arms not overhead, feet not apart and tempo drift), also synthetic-only and not selectable yet. `backend/clip_check.py` checks a mode against a recorded clip (rep count and which reps get which flags) before it can be switched on. [docs/MOVEMENT_MODES.md](docs/MOVEMENT_MODES.md) has the plan and the exercise-recognizer hook.

An exercise recognizer (auto-detect) is wired into the live loop but **off by default**. It has an experimental switch in Settings, which only works on a machine that has the model file. It only names the movement, can't switch to a mode that hasn't passed its clip check (so today it never leaves squat), and needs a model file that is not in this repo for licence reasons. See [docs/MOVEMENT_MODES.md](docs/MOVEMENT_MODES.md#recognizer-hook).
