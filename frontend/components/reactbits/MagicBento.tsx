...
const DEFAULT_GLOW_COLOR = "124, 92, 255"; // LiftGuard brand violet, as an rgb() triple
...
// The app's real sections, not decorative demo content. Card 3 and 4 get the
// large spans in the desktop grid (see MagicBento.css nth-child rules), so
// the two most-used destinations sit there deliberately. Default set for the
// Dashboard's "Explore" grid — other pages pass their own smaller `cards`
// prop (see Users page's 4-card quick-actions grid).
export const DEFAULT_BENTO_CARDS: CardDatum[] = [
  ...
  {
    title: "Hardware",
    description: "USB or WiFi pan-tilt laser pointer, connect and calibrate",
    label: "Laser",
    href: "/hardware",
  },
  {
    title: "Register User",
    description: "Browser-webcam face enrollment — no physical camera rig needed",
    label: "Identity",
    href: "/register",
  },
  {
    title: "Reports",
    description: "Per-session summaries, live or from history",
    label: "Reports",
    href: "/reports",
  },
];
...
const MagicBento = ({
  ...
  return (
    <>
      ...
      <div className={`card-grid bento-section${variant === "compact" ? " card-grid--compact" : ""}`} ref={gridRef}>
        {cards.map((card, index) => {
          ...
          return (
            <Link key={card.href + index} href={card.href} className="magic-bento-card__link">
              {enableStars ? (
                <ParticleCard
