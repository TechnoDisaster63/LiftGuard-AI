...
- **Session reports.** The stored/returned session report (`get_session_report`, saved on Stop) builds its Exercise summary from the calibrated squat counter (total reps, reps with form-risk flags, average rep time, depth, range of motion, rejected candidates) and its Fatigue summary from the rep-based indicator. The legacy tracker reported 0 reps for the same 15-rep session. With `validated_claims_only`, reports also drop the IRI history/summary and uncertainty summary.
